...
def lstm_price_forecaster() -> str:
    ...
    forecasts = []
    for ticker in PORTFOLIO:
        # FIX 1: moved display_name here so it's always defined before the try block
        display_name = TICKER_DISPLAY[ticker]
        try:
            # FIX 2: was TICKER_DISPLAY[t] — 't' is undefined; now correctly uses 'ticker'
            ...
            # FIX 3: display_name is now always defined, so this won't throw a secondary NameError
            forecasts.append(f"**{display_name} ({ticker})**: Forecast failed — {e}")
    return "\n\n".join(forecasts) if forecasts else "No forecasts."


# FIX 4: pass llm=llm to every Agent — without this, CrewAI defaults to OpenAI and ignores your LLM config
...
technical_task = Task(
    ...
    expected_output="Raw text output from the cross checker and backtester tools.",  # FIX 5: required in CrewAI >= 0.28
    agent=technical_analyst,
)

news_task = Task(
    description="Run the news fetcher tool and return only the raw output.",
    expected_output="Raw text output from the news fetcher tool.",  # FIX 5
...
forecast_task = Task(
    ...
    expected_output="A complete markdown report combining technical signals, news, and LSTM forecasts.",  # FIX 5
    # FIX 6: context links this task to the previous two so the agent can see their outputs
...
forecast_task = Task(
    description="Run the LSTM forecaster and create the final clean report.",
    agent=forecast_agent
)
